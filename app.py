...
# Page configuration
st.set_page_config(page_title="Automotive Cybersecurity Scanner Prototype",
                   page_icon="⚡",
                   layout="wide")
...
def main():
    ...
    st.title("Automotive Cybersecurity Scanner Prototype")
    st.markdown("---")
    ...
    input_method = st.radio("Choose how to specify the vehicle:",
...
def handle_vin_input():
    ...
    vin = st.text_input(
        "VIN (17 characters):",
        max_chars=17,
        help="Enter the 17-character Vehicle Identification Number").upper(
        ).strip()
        ...
        with st.spinner("Decoding VIN..."):
            try:
                ...
                if vehicle_info:
                    ...
                    st.error(
                        "Could not decode VIN. Please verify the VIN and try again."
                    )
                    return None
...
def display_vehicle_info(vehicle_info):
    ...
    with col4:
        source = "VIN Decode" if vehicle_info.get(
            'source') == 'vin' else "Manual Selection"
...
def search_vulnerabilities(vehicle_info):
    ...
    with col2:
        max_results = st.selectbox("Results Limit",
        ...
        with st.spinner("Searching CVE database..."):
            try:
                ...
                if results:
                    ...
                    st.info(
                        "No specific CVEs found for this vehicle. This doesn't mean the vehicle is secure - it may indicate limited public vulnerability data."
                    )
...
def display_cve_results(results, severity_filter):
    ...
    st.info(
        f"Found {len(filtered_results)} vulnerabilities matching your criteria"
    )

    # Display results
    for i, cve in enumerate(filtered_results):
        with st.expander(
                f"🚨 {cve['id']} - {cve.get('severity', 'Unknown')} Severity"):
            ...
            with col1:
                ...
                if cve.get('references'):
                    st.write("**References:**")
                    for ref in cve[
                            'references'][:3]:  # Limit to first 3 references
                        st.write(f"- {ref}")
